package site.pyyf.FileSystem.controller;
...
import site.pyyf.FileSystem.entity.Page;
import site.pyyf.FileSystem.service.IFileSystemService;
import site.pyyf.FileSystem.service.IFileSystemService;
import site.pyyf.FileSystem.util.CookieUtil;
import site.pyyf.FileSystem.util.MultipartSerializable;
...
@Controller
public class FileSystemCookieController   {

    @Autowired
    IFileSystemService iFileSystemService;
    
    private static final Logger logger = LoggerFactory.getLogger(FileSystemCookieController.class);
    ...
    public String mkdir(@RequestParam(value = "dirname") String dirname
        ...
        if (realPath.equals("index")) {
            ...
            iFileSystemService.insertFileSystem(dirname, currentCloudPath + "*" + dirname);

        } else {
            page.setPath("/shareFiles/show" );
            iFileSystemService.insertFileSystem(realPath + "/" + dirname, currentCloudPath + "*" + dirname);
        }


        int size = iFileSystemService.fileSize(realPath);
        page.setRows(size);

        // files = {"dir1*dir2*dir3"}
        List<site.pyyf.community.entity.File> files = iFileSystemService.list(page.getCurrent(), page.getLimit(), realPath);
    ...
    public String getPage(Model model, Page page,@RequestParam(value="dirname",defaultValue = "") String dirname,
        ...
        List<File> files = iFileSystemService.list(page.getCurrent(), page.getLimit(), realPath);
    ...
    public String uploadFile(@RequestParam(value = "file") MultipartFile url,
        ...
        final String fastdfsName = iFileSystemService.uploadFile(MultipartSerializable.file2json(url));
        ...
        iFileSystemService.insertFileSystem(absolutePath, fastdfsName);


        int size = iFileSystemService.fileSize(currentCloudPath.replace("*", "/"));
        page.setRows(size);

        // files = {"dir1*dir2*dir3"}
        List<File> files = iFileSystemService.list(page.getCurrent(), page.getLimit(), currentCloudPath.replace("*", "/"));
    ...
    public String deleteFileAndDir(HttpServletRequest request,@RequestParam("fileName") String fileName) {
        ...
        if(!fileName.equals("")){
            if(currentCloudPath.equals("index"))
                iFileSystemService.deleteFileSystem(fileName);
            else
                iFileSystemService.deleteFileSystem(currentCloudPath.replace("*","/")+"/"+fileName);
        }
        return "redirect:/shareFiles/show";
    }
    ...
    public String modifyFileAndDir(HttpServletRequest request,@RequestParam("oldFileName") String oldFileName,
        ...
        if(!oldFileName.equals("") && !newFileName.equals("")) {
            if(currentCloudPath.equals("index"))
                iFileSystemService.modifyFileSystem(oldFileName,newFileName);
            else
                iFileSystemService.modifyFileSystem(currentCloudPath.replace("*","/")+"/"+oldFileName,
    ...
    public String cleanDeletedFiles() {
        List<File> files = iFileSystemService.selectDeletedFiles();

        files.stream().forEach(f -> {
            iFileSystemService.cleanFileSystem(f.getFastdfsName());
        });
        return "redirect:/shareFiles/show";
    }
}
